...
export default function page() {
    ...
    const { userInfo, setUserInfo } = useContextData()
    ...
    const submitForm = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        try {
            const res = await signInWithEmailAndPassword(auth, logInfo.email, logInfo.password)
            if (!res.user) toast.error('No se pudo iniciar sesión')
